...
const FysiotherapieModal = () => {
  ...
  const [form, setForm] = useState({
    voornaam: "", achternaam: "", geslacht: "", adres: "", postcode: "",
    woonplaats: "", email: "", telefoon: "", verzekering: "", bsn: "", avg: false,
  });
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-foreground/50 backdrop-blur-sm" onClick={handleClose}>
      <div className="bg-card rounded-xl shadow-modal w-full max-w-lg max-h-[90vh] overflow-y-auto modal-animate" onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between p-5 border-b border-border sticky top-0 bg-card rounded-t-xl z-10">
          <h2 className="font-serif font-bold text-xl text-primary">Afspraak Fysiotherapie</h2>
        ...
        {submitted ? (
          ...
          <form onSubmit={handleSubmit} className="p-5 space-y-4">
            {[
              ...
              { label: "BSN Nummer", key: "bsn", required: true },
              ...
              <div className="flex gap-4">
                {["Man", "Vrouw"].map(g => (
                  <label key={g} className="flex items-center gap-2 text-sm cursor-pointer">
                    <input type="radio" required name="geslacht-fysio" value={g} checked={form.geslacht === g} onChange={e => setForm({ ...form, geslacht: e.target.value })} className="accent-primary" />
                    {g}
                  </label>
                ))}
              </div>
