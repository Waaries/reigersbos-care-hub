...
import { X, CheckCircle, User, ChevronRight, Shield } from "lucide-react";
...
const DietetiekModal = () => {
  ...
  const [form, setForm] = useState({
    ...
    telefoon: "", email: "",
    verzekering: "", reden: "", opmerking: "", avg: false,
  });
  ...
  const steps = [
    { id: 1, label: "Uw gegevens", icon: User },
    { id: 2, label: "Reden & Akkoord", icon: Shield },
  ];
          ...
          {submitted ? (
            <div style={{ padding: "48px 32px", textAlign: "center" }}>
              ...
              <p style={{ fontSize: "14px", color: "#64748B", lineHeight: 1.6, marginBottom: "24px" }}>
                Wij nemen zo spoedig mogelijk contact met u op voor uw diëtetiek afspraak.
              </p>
              <button
                onClick={handleClose}
                style={{
            ...
            <form onSubmit={handleSubmit}>
              <div style={{ padding: "28px 32px", display: "flex", flexDirection: "column", gap: "16px" }}>
                    ...
                    <div style={{ display: "flex", gap: "8px" }}>
                      {["Man", "Vrouw", "Anders"].map(g => (
                        <label key={g} style={{
                          ...
                          <input type="radio" required name="geslacht-dietetiek" value={g} checked={form.geslacht === g} onChange={e => set("geslacht", e.target.value)} style={{ display: "none" }} />
                          {g}
                        </label>
                      ))}
                    </div>
                ...
                {step === 2 && (<>
                  <div>
                    <label className={labelClass}>Reden van aanmelding *</label>
                    <p style={{ fontSize: "12px", color: "#64748B", marginBottom: "10px" }}>Waarvoor wilt u een afspraak maken?</p>
                    <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
                      {[
                        ...
                        <label key={r} style={{
                          ...
                          border: `2px solid ${form.reden === r ? "#0EA5A0" : "#E2E8F0"}`,
                          background: form.reden === r ? "#F0FDFA" : "#F8FAFC",
                          fontSize: "13px", fontWeight: 600, color: form.reden === r ? "#0EA5A0" : "#64748B",
                          transition: "all 0.2s",
                        }}>
                          <div style={{
                            width: "18px", height: "18px", borderRadius: "50%", flexShrink: 0,
                            border: `2px solid ${form.reden === r ? "#0EA5A0" : "#CBD5E1"}`,
                            background: form.reden === r ? "#0EA5A0" : "white",
                            display: "flex", alignItems: "center", justifyContent: "center",
                            transition: "all 0.2s",
                          }}>
                            {form.reden === r && <div style={{ width: "6px", height: "6px", borderRadius: "50%", background: "white" }} />}
                          </div>
                          <input type="radio" required name="reden-dietetiek" value={r} checked={form.reden === r} onChange={e => set("reden", e.target.value)} style={{ display: "none" }} />
                          {r}
                        </label>
                      ))}
                    </div>
                  ...
                  <div>
                    ...
                    <textarea className={inputClass} value={form.opmerking} onChange={e => set("opmerking", e.target.value)} placeholder="Eventuele aanvullende informatie..." rows={3} style={{ resize: "none" }} />
                  </div>
                  <label style={{
                  ...
                  <div style={{ background: "#F0FDFA", border: "1px solid #99F6E4", borderRadius: "12px", padding: "14px" }}>
                    <p style={{ fontSize: "12px", color: "#0F766E", lineHeight: 1.6, margin: 0 }}>
                      🥗 <strong>Persoonlijk voedingsadvies op maat.</strong> Wij nemen binnen 2 werkdagen contact op om een afspraak in te plannen.
                    </p>
                  </div>
                </>)}
              </div>
              ...
              <div style={{ padding: "16px 32px 28px", display: "flex", gap: "12px", justifyContent: "space-between", borderTop: "1px solid #F1F5F9" }}>
                <button type="button" onClick={() => step > 1 ? setStep((step - 1) as Step) : handleClose()} style={{
